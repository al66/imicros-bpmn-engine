...
class Parser {
    ...
    mapId(extern,fix) {
        ...
        if (!this.idMap[extern]) this.idMap[extern] = fix || uuid();
    ...
    parse({ id, xmlData, objectName, ownerId, core = false }) {
        ...
        this.toArray(jsonObj.definitions?.process).filter(p => p._isExecutable === true).forEach(bpmnProcess => {
            ...
            parsedProcess.process = {
                ...
                id: this.mapId(bpmnProcess._id ?? null,id),
                localCollaborationId: jsonObj.definitions?.collaboration?._id || null,
                localProcessId: bpmnProcess._id,
                versionTag: bpmnProcess.extensionElements?.versionTag?._value || null,
            ...
            parsed.processCollection.push(parsedProcess);
        });

        // add id map
        parsed.idMap = this.idMap;
